...
namespace MPS.Ledger_Setup
{
    public partial class LedgerSearchFrm : Form
    {
        ...
        public void LoadData()
        {
            ...
            customerList = (from c in mbmsEntities.Customers
                            where c.Active == true && 
                            (c.Ledger.BookCode == bookCode && c.PageNo == pageNo && c.LineNo == lineNo) 
                            || (c.Ledger.BookCode==bookCode && c.PageNo==pageNo) || (c.Ledger.BookCode==bookCode)
                            select c).ToList();
            foundDataBind();
        }
        public void foundDataBind()
        {
            dgvLedgerSearchList.DataSource = "";
